...
<script lang="ts">
  ...
  if ($page.route.id?.replace("/", "") != null) {
    activeNavLi.set($page.route.id?.replace("/", ""));
  }
...
<Navbar let:hidden let:toggle>
  ...
  <NavUl {hidden}>
    ...
    <NavLi href="/" active={$activeNavLi === ''} on:click={() => handleClick('')}>
      Home
    </NavLi>
    <NavLi href="/resume" active={$activeNavLi === 'resume'} on:click={() => handleClick('resume')}>
      Resume
    </NavLi>
    <NavLi href="/blog" active={$activeNavLi === 'blog'} on:click={() => handleClick('blog')}>
      Blog
    </NavLi>
    <NavLi href="/contact" active={$activeNavLi === 'contact'} on:click={() => handleClick('contact')}>
      Contact
    </NavLi>
  </NavUl>
</Navbar>
